...
_uapi(){
  local cur prev opts line base
    ...
    line="${COMP_LINE}"
    base="${1##*/}"
    opts="--help --output --user $(uapi --help 2>&1 | grep :: | cut -d: -f1 | sort | uniq)"
    #opts="--help --output --user Backup Bandwidth Batch Brand Branding CacheBuster Chkservd Chrome Contactus cPAddons cPGreyList DAV DCV DNS DNSSEC DomainInfo Email ExternalAuthentication Features Fileman Ftp Integration LangPHP LastLogin Locale Mailboxes Market Mime ModSecurity Mysql Notifications NVData Parser PassengerApps PasswdStrength Postgresql Pushbullet Quota Resellers ResourceUsage Restore Session SiteTemplates SpamAssassin SSH SSL StatsBar Styles Themes TwoFactorAuth UserManager UserTasks VersionControl VersionControlDeployment WebDisk WebmailApps WebVhosts WordPressInstanceManager"
...
Mailboxes )
  COMPREPLY=( $(compgen -W "expunge_mailbox_messages expunge_messages_for_mailbox_guid get_mailbox_status_list" -- ${cur}) )
  ...
  expunge_mailbox_messages )
    ...
    case ${prev} in
      account )
        COMPREPLY=( $(compgen -W "$(_uapi_cpuser; _uapi_emailaddress)" -- ${cur}) )
        return 0 ;;
      mailbox )
        COMPREPLY=( $(compgen -W "" -- ${cur}) )
        return 0 ;;
